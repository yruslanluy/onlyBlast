...
contract ERC721 is ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    function name() external pure override returns (string memory) {
        return "Only Blast: Proof of Contribution";
    }
...
contract onlyBlastNFT is Ownable, IERC2981, ERC721 {
    ...
    mapping (address => uint32) public spot;
    ...
    function mint() external {
        ...
        uint tokenId = spot[msg.sender];
        require(tokenId != 0, "Mint is not allowed for you!");
     ...
     * @dev enables/disables mint functions
     */
    // mint is manual switchable only in testnet for tests
    function togglePublicSale() external onlyOwner {
        _mintingEnabled = !_mintingEnabled;
    }

    function addToWhitelist(address[] calldata toAddAddresses, uint32[] calldata spotNumber) public onlyOwner
    {
        for (uint i = 0; i < toAddAddresses.length; i++) {
            spot[toAddAddresses[i]] = spotNumber[i];
        }
    }
}
